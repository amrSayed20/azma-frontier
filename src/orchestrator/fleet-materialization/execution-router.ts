 ...
 * File: src/orchestrator/fleet-materialization/execution-router.ts
 * * The central execution bridge for the Fleet Materialization Runtime.
...
export class ExecutionRouter {
  ...
  public static async routeToDirector(input: DirectorInput): Promise<DirectorOutput> {
    ...
    try {
      ...
      // The result (success or failure) is returned to the higher Fleet Materialization Runtime state manager
      // so it can eventually be routed to the Phase 4 Production Chambers.
      return directorResult;

    } catch (error) {
      // 5. Global Failsafe
      // Ensure that unexpected routing or chamber crashes do not bring down the Fleet Materialization Runtime.
      return {
        success: false,
